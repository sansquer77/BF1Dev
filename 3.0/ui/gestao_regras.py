...
def main():
    ...
    tabs = st.tabs(["Regras por Temporada",
                    "Criar/Editar Regras",
    ...
    with tabs[0]:
        ...
        if not regras_disponiveis:
            st.warning(
                "Nenhuma regra cadastrada. Crie uma regra na aba 'Criar/Editar Regras'.")
            # NÃO usar return aqui - apenas não mostra o resto desta aba
        else:
            regras_nomes = {r['id']: r['nome_regra']
                            for r in regras_disponiveis}
            ...
            with col_temp:
                temporada_selecionada = st.selectbox(
                    "Selecione a Temporada", temporadas, key="temp_associar")
            with col_regra:
            ...
            if st.button("Associar Regra à Temporada"):
                if regra_selecionada is None:
                    ...
                    st.success(
                        f"Regra '{
                            regras_nomes[regra_selecionada]}' associada à temporada {temporada_selecionada}!")
                    st.rerun()
                else:
            ...
            st.caption(
                "Reprocessa pontos somente para a temporada selecionada.")
            current_year = int(get_current_year_str())
            temporadas_recalc = get_season_options(
                fallback_years=[str(current_year - 1), str(current_year), str(current_year + 1)],
                include_current_year=True,
            )
            temporada_recalc = st.selectbox(
                "Temporada para recalcular",
                temporadas_recalc,
                key="temp_recalcular")
            if st.button(
                "Recalcular pontuação desta temporada",
                    key="btn_recalcular_pontuacao_temp"):
                try:
                    ...
                    atualizar_classificacoes_todas_as_provas(
                        temporada=temporada_recalc)
                    st.success(
                        f"Pontuação recalculada para todas as provas da temporada {temporada_recalc}!")
    ...
    with tabs[1]:
        ...
        if modo == "Criar Nova Regra":
            ...
            if not regras_existentes:
                ...
                regras_dict = {r['nome_regra']: r['id']
                               for r in regras_existentes}
                regra_nome = st.selectbox(
                    "Selecione a Regra para Editar", list(
                        regras_dict.keys()))
                try:
    ...
    with tabs[2]:
        ...
        if not regras_existentes:
            ...
            if not temporadas:
                st.warning(
                    "Nenhuma temporada cadastrada na tabela 'temporadas'. Crie a temporada antes de definir pontos por posição.")
                st.stop()
            temporada_sel = st.selectbox(
                "Temporada", temporadas, key="pont_temp_select")
            regra_atual = get_regra_temporada(temporada_sel)
            if not regra_atual:
                st.warning(
                    "Nenhuma regra associada à temporada selecionada. Associe uma regra na aba 'Regras por Temporada'.")
                st.stop()
            st.info(
                f"Regra atual da temporada {temporada_sel}: {
                    regra_atual['nome_regra']}")
            tipo_prova = st.selectbox(
                "Tipo de Prova", [
                    "Normal", "Sprint"], key="pont_tipo_select")
            # Carrega lista atual
            pts_lista = regra_atual.get(
                'pontos_sprint_posicoes' if tipo_prova == 'Sprint' else 'pontos_posicoes', [])
            ...
            qtd_default = len([x for x in pts_lista if x and int(x) > 0]) or (
                8 if tipo_prova == 'Sprint' else 10)
            qtd_positions = st.number_input(
                "Quantidade de posições que pontuam",
                min_value=1,
                max_value=20,
                value=qtd_default)
            ...
            for i in range(qtd_positions):
                col = cols[i % 5]
                with col:
                    val = st.number_input(f"{i + 1}º",
                                          min_value=0,
                                          max_value=1000,
                                          value=int(pts_lista_pad[i]),
                                          key=f"pt_pos_{tipo_prova}_{i + 1}")
            ...
            if st.button(
                "Salvar Tabela de Pontos (apenas para esta temporada)",
                type="primary",
                    key="btn_salvar_pontos_posicoes"):
                try:
                    # Se a regra é compartilhada entre múltiplas temporadas,
                    # clonar antes de atualizar
                    temporadas_usando = listar_temporadas_por_regra(
                        regra_atual['id'])
                    regra_id_target = regra_atual['id']
                    if len(
                            temporadas_usando) > 1 and temporada_sel in temporadas_usando:
                        novo_nome = f"{
                            regra_atual['nome_regra']} - {temporada_sel}"
                        new_id = clonar_regra(regra_atual['id'], novo_nome)
                        if not new_id:
                            st.error(
                                "Falha ao clonar regra para atualização específica da temporada.")
                            st.stop()
                        # Reassocia a temporada selecionada ao clone
                        if not associar_regra_temporada(temporada_sel, new_id):
                            st.error(
                                "Falha ao associar regra clonada à temporada.")
                            st.stop()
    ...
    try:
        ...
        with st.form("form_regra", clear_on_submit=False):
            ...
            with col1:
                ...
                # 3. Mesma Equipe
                mesma_equipe_default = safe_get(
                    regra_atual, 'mesma_equipe', False)
            ...
            with col2:
                ...
                penalidade_default = safe_get(
                    regra_atual, 'penalidade_abandono', False)
                ...
                wildcard_default = safe_get(
                    regra_atual, 'pontos_dobrada', False)
            ...
            if submitted:
                ...
                if fichas_por_piloto > quantidade_fichas:
                    st.error(
                        f"❌ Fichas por piloto ({fichas_por_piloto}) não pode ser maior que o total ({quantidade_fichas})")
                    return

                if qtd_minima_pilotos > (
                        quantidade_fichas // fichas_por_piloto):
                    st.warning(
                        f"⚠️ Aviso: mínimo de {qtd_minima_pilotos} pilotos com máx {fichas_por_piloto} fichas cada pode ser impossível com {quantidade_fichas} fichas totais")
                ...
                try:
                    ...
                    if sucesso:
                        st.success(
                            f"✅ Regra '{nome_regra}' {msg_tipo} com sucesso!")
                        st.balloons()
                        st.rerun()
                    else:
                        st.error(
                            f"❌ Erro ao salvar. Verifique se o nome já existe.")
                except Exception as e:
...
def excluir_regra_form(regras_existentes):
    ...
    regra_nome = st.selectbox(
        "Selecione a Regra para Excluir", list(
            regras_dict.keys()), key="excluir_regra_select")
