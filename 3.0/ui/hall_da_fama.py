...
def hall_da_fama():
    ...
    with db_connect() as conn:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT DISTINCT temporada FROM hall_da_fama ORDER BY temporada DESC")
        ...
        for _, user in usuarios.iterrows():
            ...
            for season in seasons:
                ...
                if result and result[0] is not None:
                    pos_val = result[0]
                    pts_val = result[1] if len(
                        result) > 1 and result[1] is not None else 0
            ...
            if st.button(
                ...
                if not entries:
                    ...
                    for entry in entries:
                        try:
                            user_id = usuarios[usuarios['nome']
                                               == entry['user']]['id'].values[0]
                            # Garantir tipo INTEGER para o banco
                            user_id = int(user_id)
                            ...
                            if existing:
                                errors.append(
                                    f"⚠️ **{entry['user']}** já possui registro para {season_year}")
                            else:
                                c.execute(
                ...
                with st.container():
                    ...
                    with col5:
                        if st.button(
                            ...
                            try:
                                c.execute(
                                    "DELETE FROM hall_da_fama WHERE id = ?", (record_id,))
                                conn.commit()
                                st.success(
