...
def participante_view():
    ...
    if not force_change:
        with tabs[0]:
            # Betting form should show only provas that will occur in the
            ...
            # then restrict by the prova date to ensure only
            # upcoming/current-year events are shown.
            provas_df = get_provas_df(temporada)
            try:
                if not provas_df.empty and 'data' in provas_df.columns:
                    provas_df['__data_dt'] = pd.to_datetime(
                        provas_df['data'], errors='coerce')
                    provas = provas_df[provas_df['__data_dt'].apply(
                ...
                if len(provas) > 0 and len(pilotos_df) > 2:
                    ...
                    with col_btn:
                        if st.button("Ver regras", width="stretch"):
                            ...
                            regras_sel = get_regras_aplicaveis(
                                temporada, tipo_sel)
                            _mostrar_regras_dialog(
                                regras_sel, temporada, tipo_sel)
                    with col_sem_ideias:
                    ...
                    quantidade_fichas = int(
                        regras.get('quantidade_fichas', 15))
                    min_pilotos_regra = int(
                        regras.get(
                            ...
                            regras.get(
                                'min_pilotos',
                                3)))
                    fichas_max_por_piloto = int(regras.get(
                        'fichas_por_piloto', quantidade_fichas))
                    permite_mesma_equipe = bool(
                        regras.get('mesma_equipe', False))
                    ...
                    prova_id_form = st.session_state.get(
                        "aposta_form_prova_id")
                    ...
                    st.write(
                        "Escolha seus pilotos e distribua suas fichas entre eles de acordo com as regras:")
                    ...
                    pilotos_validos = [
                        p for p in pilotos_aposta if p != "Nenhum"]
                    ...
                    if st.button("Efetivar Aposta"):
                        ...
                        if not permite_mesma_equipe and len(
                                set(equipes_apostadas)) < len(equipes_apostadas):
                            erros.append(
                                "Não é permitido apostar em dois pilotos da mesma equipe.")
                        if len(pilotos_validos) < min_pilotos_regra:
                            erros.append(
                                f"Você deve apostar em pelo menos {min_pilotos_regra} pilotos.")
                        if total_fichas != quantidade_fichas:
                            erros.append(
                                f"A soma das fichas deve ser exatamente {quantidade_fichas}.")
                        if fichas_validas and max(
                                fichas_validas) > fichas_max_por_piloto:
                            erros.append(
                ...
                for aba, (_, aposta) in zip(abas, apostas_part.iterrows()):
                    with aba:
                        ...
                        if regras.get(
                            ...
                            if abandonos:
                                num_aband = sum(
                                    1 for p in pilotos_apostados if p.strip() in abandonos)
                        ...
                        if penalidade_abandono:
                            st.write(
                                f"**Penalidade por abandono:** -{penalidade_abandono}")
                        if penalidade_auto:
                            st.write(
        ...
        try:
            with db_connect() as conn:
                df_posicoes = pd.read_sql(
                    'SELECT prova_id, usuario_id, posicao, temporada FROM posicoes_participantes',
                    conn)
        except Exception:
            st.info(
