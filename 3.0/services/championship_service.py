...
def can_place_championship_bet(season: int | None = None,
                               now: datetime | None = None) -> tuple[bool,
                                                                     str,
                                                                     datetime | None]:
...
def get_championship_bets_df(season: int | None = None):
    ...
    with db_connect() as conn:
        if season is None:
            ...
            df = pd.read_sql(
                'SELECT user_id, user_nome, champion, vice, team, season, bet_time FROM championship_bets WHERE season = ?',
                conn,
                params=(
                    season,
                ))
...
def get_championship_bets_log_df(season: int | None = None):
    ...
    with db_connect() as conn:
        if season is None:
            ...
            df = pd.read_sql(
                'SELECT user_id, user_nome, champion, vice, team, season, bet_time FROM championship_bets_log WHERE season = ?',
                conn,
                params=(
                    season,
                ))
...
def get_championship_results_df(season: int | None = None):
    ...
    with db_connect() as conn:
        if season is None:
            df = pd.read_sql(
                'SELECT season, champion, vice, team FROM championship_results', conn)
        else:
            df = pd.read_sql(
                'SELECT season, champion, vice, team FROM championship_results WHERE season = ?',
                conn,
                params=(
                    season,
                ))
