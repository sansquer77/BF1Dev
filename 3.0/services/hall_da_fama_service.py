    ...
    try:
        with db_connect() as conn:
            ...
            c.execute("""UPDATE posicoes_participantes
                   ...
                   WHERE id = ?""", (new_posicao, new_temporada,
...
def deletar_resultado_historico(registro_id: int) -> dict:
    ...
    try:
        with db_connect() as conn:
            ...
            # Delete record
            c.execute(
                "DELETE FROM posicoes_participantes WHERE id = ?", (registro_id,))
            conn.commit()
        ...
        with db_connect() as conn:
            ...
            for idx, item in enumerate(dados):
                try:
                    ...
                    # Insert new record
                    c.execute("""INSERT INTO posicoes_participantes
                           (usuario_id, posicao, temporada, data_atualizacao)
                           VALUES (?, ?, ?, ?)""", (usuario_id, int(posicao),
                                                    str(temporada), datetime.now().isoformat()))
                    imported += 1
