...
def _aposta_valida_regras(
    ...
    if not permite_mesma_equipe and not pilotos_df.empty and 'equipe' in pilotos_df.columns:
        mapa_eq = dict(zip(pilotos_df['nome'].astype(
            str), pilotos_df['equipe'].astype(str)))
...
def _get_resumo_ultimas_apostas(
    ...
    if not provas_df.empty and 'id' in provas_df.columns and 'nome' in provas_df.columns:
        dict(zip(provas_df['id'], provas_df['nome']))
    ...
    try:
        ...
        if not df_delta.empty:
            ...
            for _, row in df_delta.sort_values(
                    "Delta", ascending=False).head(3).iterrows():
                top_delta.append(
                    {
                        "n": str(row.get("Driver", "")).strip(),
            ...
            for _, row in df_delta.sort_values(
                    "Delta", ascending=True).head(2).iterrows():
                bottom_delta.append(
                    {
                        "n": str(row.get("Driver", "")).strip(),
                        "d": int(row.get("Delta", 0) or 0),
...
def _gerar_aposta_perplexity(
    ...
    pilotos_disponiveis = pilotos_df['nome'].astype(
        str).tolist() if not pilotos_df.empty else []
    min_pilotos = int(regras.get('qtd_minima_pilotos')
                      or regras.get('min_pilotos', 3))
    qtd_fichas = int(regras.get('quantidade_fichas', 15))
    fichas_max = int(regras.get('fichas_por_piloto', qtd_fichas))
    ...
    try:
        ...
        logger.warning(
            f"Falha na geração via Perplexity para aposta estratégica: {e}")
        return None
...
def pode_fazer_aposta(data_prova_str, horario_prova_str, horario_usuario=None):
    ...
    """
    try:
        horario_limite_sp = _parse_datetime_sp(
            data_prova_str, horario_prova_str)
        ...
        mensagem = f"Aposta {
            'permitida' if pode else 'bloqueada'} (Horário limite SP: {
...
def salvar_aposta(
    ...
    try:
        ...
        if show_errors:
            st.error(
                f"IDs inválidos: usuario_id={usuario_id}, prova_id={prova_id} ({e})")
        return False
    ...
    # Determinar tipo da prova usando coluna `tipo` quando disponível;
    # fallback por nome
    try:
        ...
        with db_connect() as conn:
            ...
            if tipo_aposta == 0 or permitir_salvar_tardia:
                if 'temporada' in aposta_cols:
                    c.execute(
                        'DELETE FROM apostas WHERE usuario_id=? AND prova_id=? AND temporada=?',
                        (usuario_id,
                         prova_id,
                         temporada))
                else:
                ...
                try:
                    ...
                    enviar_email(
                        usuario['email'],
                        f"Aposta não efetivada - {nome_prova_bd}",
                        corpo_email)
                except Exception as e:
                    logger.warning(
                        f"Falha ao enviar email de aposta rejeitada para {
                            usuario.get('email')}: {e}")
                try:
                    registrar_log_aposta(
                    ...
                    logger.warning(
                        f"Falha ao registrar log de aposta rejeitada para {
                            usuario.get('email')}: {e}")
                return False
            conn.commit()
            ...
            try:
                ...
                logger.warning(
                    f"Falha ao enviar email de aposta para {
                        usuario.get('email')}: {e}")
...
def gerar_aposta_aleatoria_com_regras(pilotos_df, regras: dict):
    ...
    if len(equipes_unicas) >= alvo_pilotos:
        ...
        # Não há equipes suficientes; se permitido, escolhe pilotos adicionais
        # de equipes repetidas
...
def _determinar_tipo_prova(
        prova_row: pd.Series | dict,
        nome_prova: str | None) -> str:
    try:
...
def gerar_aposta_automatica(
    ...
    if not ap_ant.empty:
        ...
        # Ajustar aposta copiada para obedecer regras da prova atual (ex.:
        # Sprint x Normal)
    ...
    return (
        True,
        "Aposta automática gerada!") if sucesso else (
        False,
        "Falha ao salvar.")
...
def gerar_aposta_sem_ideias(usuario_id, prova_id, nome_prova, temporada=None):
    ...
    ultimas_apostas = _get_resumo_ultimas_apostas(
        usuario_id, apostas_df, provas_df, limite=2)
...
def calcular_pontuacao_lote(ap_df, res_df, prov_df, temporada_descarte=None):
    ...
    for _, r in res_df.iterrows():
        ...
        try:
            if 'abandono_pilotos' in res_df.columns:
                ...
                aband_list = [p.strip()
                              for p in str(raw).split(',') if p and p.strip()]
    ...
    temporadas_prova = dict(zip(prov_df['id'], prov_df['temporada'] if 'temporada' in prov_df.columns else [
                            str(datetime.now().year)] * len(prov_df)))
    ...
    for _, aposta in ap_df.iterrows():
        ...
        if has_temp_aposta:
            try:
                ...
                temporada_aposta = None
        if temporada_aposta is not None and str(
                temporada_aposta).strip() != "" and not pd.isna(temporada_aposta):
            temporada_prova = str(temporada_aposta)
        else:
            temporada_prova = temporadas_prova.get(
                prova_id, str(datetime.now().year))
        ...
        # Corridas Sprint sempre usam a tabela de sprint; regra_sprint só afeta
        # fichas/minimo, não a tabela.
        if tipo == 'Sprint':
        ...
        # Observação: multiplicador de sprint será aplicado APÓS bônus e
        # penalidades
        pt = 0
        ...
        # Deduz `pontos_penalidade` por cada piloto apostado que esteja na
        # lista de abandonos
        if regras.get('penalidade_abandono'):
            aband_prova = abandonos_map.get(prova_id, set())
            if aband_prova:
                # Conta apenas abandonos dentre os pilotos apostados (exclui
                # palpite do 11º)
                num_aband_apostados = sum(
                    1 for p in pilotos if p in aband_prova)
                deduz = regras.get(
                    'pontos_penalidade',
                    0) * num_aband_apostados
                if deduz:
                    pt -= deduz
...
def salvar_classificacao_prova(p_id, df_c, temp=None):
    ...
    with db_connect() as conn:
        ...
        if has_temporada:
            c.execute(
                'DELETE FROM posicoes_participantes WHERE prova_id=? AND temporada=?',
                (p_id,
                 temp))
        else:
            c.execute(
                'DELETE FROM posicoes_participantes WHERE prova_id=?', (p_id,))
...
def atualizar_classificacoes_todas_as_provas(temporada: str | None = None):
    with db_connect() as conn:
        usrs = pd.read_sql(
            'SELECT id FROM usuarios WHERE status = "Ativo"', conn)
        provs = pd.read_sql(
            'SELECT id, nome, data, tipo, temporada FROM provas', conn)
        apts = pd.read_sql(
            'SELECT usuario_id, prova_id, data_envio, pilotos, fichas, piloto_11, automatica, temporada FROM apostas',
            conn)
        ress = pd.read_sql(
            'SELECT prova_id, posicoes, abandono_pilotos FROM resultados', conn)
        ...
        if not provs.empty:
            if 'temporada' in provs.columns and 'data' in provs.columns:
                provs_dt = provs.copy()
                provs_dt['__data_dt'] = pd.to_datetime(
                    provs_dt['data'], errors='coerce')
                for temp_val, grp in provs_dt.groupby('temporada'):
                    grp = grp.sort_values('__data_dt')
                    if not grp.empty:
                        primeira_prova_por_temp[str(temp_val)] = int(
                            grp.iloc[0]['id'])
                ...
                if not provs_dt.empty:
                    primeira_prova_por_temp[str(datetime.now().year)] = int(
                        provs_dt.iloc[0]['id'])
        ...
        for _, pr in provs.iterrows():
            ...
            # Aplicar regra de 85% do pior pontuador na primeira corrida sem
            # base
            if first_no_base_flags:
                try:
                    ...
                    pior_pontuador = min(
                        pontos_validos) if pontos_validos else 0
                except Exception:
                    pior_pontuador = 0
                for t in tab:
