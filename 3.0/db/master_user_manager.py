...
class MasterUserManager:
        ...
        try:
            ...
            try:
                if hasattr(st, 'secrets') and st.secrets:
                    ...
                    nome = MasterUserManager._get_env_value(
                        MasterUserManager.ENV_VAR_NOME, secrets_dict)
                    email = MasterUserManager._get_env_value(
                        MasterUserManager.ENV_VAR_EMAIL, secrets_dict)
                    senha = MasterUserManager._get_env_value(
                        MasterUserManager.ENV_VAR_SENHA, secrets_dict)
                    ...
                    if nome and email and senha:
                        return {
                            ...
                            'email': email,
                            'senha': senha,
                            'telegram': MasterUserManager._get_env_value(
                                MasterUserManager.ENV_VAR_TELEGRAM,
                                secrets_dict)}
            ...
            nome = MasterUserManager._get_env_value(
                MasterUserManager.ENV_VAR_NOME, env_dict)
            email = MasterUserManager._get_env_value(
                MasterUserManager.ENV_VAR_EMAIL, env_dict)
            senha = MasterUserManager._get_env_value(
                MasterUserManager.ENV_VAR_SENHA, env_dict)
    ...
    def _create_master() -> bool:
        ...
        if not creds:
            logger.info(
                "ℹ️  Variáveis de ambiente do Master não configuradas. Pulando criação automática.")
            return False
        ...
        try:
            ...
            with pool.get_connection() as conn:
                ...
                logger.info(
                    f"✓ Usuário Master criado com sucesso (ID: {master_id})")
