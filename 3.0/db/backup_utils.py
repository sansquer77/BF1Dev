...
def upload_db():
    ...
    if 'import_success' in st.session_state:
        ...
        if info.get('type') == 'db':
            st.success(
                "✅ Banco de dados .db validado e restaurado com sucesso!")
        else:
            st.success(
                f"✅ Importação concluída: {
                    info['tables']} tabelas, {
                    info['records']} registros")
            if info['errors'] > 0:
                st.warning(
                    f"⚠️ {
                        info['errors']} comandos falharam (podem ser erros esperados de sintaxe)")
    ...
    if uploaded_file is not None:
        ...
        if file_extension == 'sql':
            ...
            try:
                ...
                sql_content = re.sub(
                    r'`(\w+)`\s+integer\s+AUTO_INCREMENT',
                ...
                # CRITICAL: FULL garante persistência
                cursor.execute("PRAGMA synchronous=FULL")
                # WAL para performance mantendo segurança
                cursor.execute("PRAGMA journal_mode=WAL")
                # Cache maior para performance
                cursor.execute("PRAGMA cache_size=10000")
                # Usar memória para temp
                ...
                status_text.text(
                    f"Iniciando importação de {total} comandos...")
                ...
                for i, statement in enumerate(statements):
                    try:
                        ...
                        if i > 0 and i % 100 == 0:
                            ...
                            # Atualizar UI apenas a cada 100 comandos (não mais
                            # frequente)
                            if i - last_update >= 100:
                                progress_bar.progress(min(i / total, 0.99))
                                status_text.text(
                                    f"Importando: {i}/{total} comandos ({int(i / total * 100)}%)")
                                last_update = i
                ...
                # CRÍTICO: Consolidar WAL antes de fechar - força todos os
                # dados para o arquivo principal
                ...
                # Criar backup do banco atual usando sqlite3.backup (garante
                # WAL)
                if DB_PATH.exists():
                    ...
                    backup_file = backup_path / \
                        f"backup_antes_sql_import_{timestamp}.db"
                ...
                if not DB_PATH.exists() or DB_PATH.stat().st_size == 0:
                    raise Exception(
                        f"Erro ao salvar banco: arquivo vazio ou não existe! Path: {DB_PATH}")
            ...
            try:
                ...
                # Verificar integridade e consolidar WAL usando API Python
                # nativa
                ...
                try:
                    ...
                    if result[0] != "ok":
                        st.error(
                            f"❌ Arquivo de backup está corrompido: {
                                result[0]}")
                        source_conn.close()
                        shutil.rmtree(temp_dir)
                        return
                ...
                # Criar backup antes de sobrescrever usando sqlite3.backup
                # (garante WAL)
                if DB_PATH.exists():
                    backup_path = Path("backups")
                    backup_path.mkdir(exist_ok=True)
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    backup_file = backup_path / \
                        f"backup_antes_restauracao_{timestamp}.db"
                    src = sqlite3.connect(str(DB_PATH), timeout=30)
                    bkp = sqlite3.connect(str(backup_file), timeout=30)
                    try:
                        src.execute("PRAGMA wal_checkpoint(TRUNCATE)")
                ...
                # Sobrescrever banco com versão limpa usando sqlite3.backup
                # (garante WAL)
                ...
                # IMPORTANTE: Remover arquivos WAL/SHM antigos que podem causar
                # problemas
                wal_file = Path(str(DB_PATH) + "-wal")
                shm_file = Path(str(DB_PATH) + "-shm")
                if wal_file.exists():
                ...
                try:
                    ...
                    direct_conn.close()
                    st.info(
                        f"🔍 Arquivo direto: {usuarios_direct} usuários, {apostas_direct} apostas")
                except Exception as e:
                ...
                try:
                    with db_connect() as conn:
                        ...
                        st.info(
                            f"🔍 Pelo pool: {usuarios_pool} usuários, {apostas_pool} apostas")
                except Exception as e:
...
def upload_tabela():
    ...
    if uploaded_file is not None and tabela:
        ...
        if st.button(
            "✅ Confirmar Importação - SOBRESCREVER TODOS OS DADOS",
            type="primary",
                width="stretch"):
            try:
                ...
                # evita falhas ao limpar e regravar
                cursor.execute("PRAGMA foreign_keys=OFF")
                ...
                if not cols_info:
                    conn.close()
                    raise ValueError(
                        f"Tabela '{tabela}' não encontrada no banco.")
                ...
                if missing_cols:
                    conn.close()
                    raise ValueError(
                        f"Colunas faltantes no Excel: {missing_cols}")
                if extra_cols:
                    st.info(
                        f"ℹ️ Colunas extras no Excel serão ignoradas: {extra_cols}")
                ...
                for col_name in db_cols:
                    ...
                    if "INT" in col_type or "BOOL" in col_type:
                        coerced = pd.to_numeric(
                            series, errors='coerce').astype('Int64')
                        df_alinhado[col_name] = coerced.where(
                            ~coerced.isna(), None)
                        continue
                    if any(
                        t in col_type for t in [
                            "REAL",
                            "FLOA",
                            "DOUB",
                            "NUM",
                    ...
                    if any(t in col_type for t in ["DATE", "TIME"]):
                        coerced = pd.to_datetime(
                            series, errors='coerce', dayfirst=True)
                        formatted = coerced.dt.strftime('%Y-%m-%d %H:%M:%S')
                        df_alinhado[col_name] = formatted.where(
                            coerced.notna(), None)
                        continue
                ...
                # CRÍTICO: Fechar pool para forçar recriação após modificar
                # dados
                from db.connection_pool import close_pool
                try:
                ...
                st.info(
                    "💡 Verifique se as colunas do arquivo Excel correspondem exatamente às colunas da tabela.")
                import traceback
                st.code(traceback.format_exc())
...
def main():
    ...
    st.info(
        f"📍 Banco de dados: `{
            DB_PATH.name}` | Status: {
            'Existe' if DB_PATH.exists() else 'Não encontrado'}")
...
def create_next_temporada() -> str:
    ...
    try:
        with db_connect() as conn:
            c = conn.cursor()
            c.execute(
                "INSERT OR IGNORE INTO temporadas (temporada) VALUES (?)", (str(next_year),))
            conn.commit()
    except Exception as e:
...
def list_temporadas() -> list:
    ...
    try:
        with db_connect() as conn:
            c = conn.cursor()
            c.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='temporadas'")
            if not c.fetchone():
                return []
