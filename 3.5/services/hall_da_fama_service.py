    ...
    try:
        with db_connect() as conn:
            ...
            if not isinstance(posicao, int) or posicao < 1 or posicao > 1000:
                return {
                    "success": False,
                    "message": "Posição deve estar entre 1 e 1000"}

            if not isinstance(temporada, str) or not temporada.strip():
                return {
                    "success": False,
                    "message": "Temporada não pode estar vazia"}
            ...
            logger.info(
                f"✅ Resultado adicionado: usuario_id={usuario_id}, posicao={posicao}, temporada={temporada}")
        ...
        return {
            "success": False,
            "message": f"Erro ao adicionar resultado: {
                str(e)}"}


def editar_resultado_historico(
        registro_id: int,
        posicao: Optional[int] = None,
        temporada: Optional[str] = None) -> dict:
    """
    ...
    try:
        with db_connect() as conn:
            ...
            usuario_id, current_posicao, current_temporada = existing[
                'usuario_id'], existing['posicao'], existing['temporada']
            ...
            # Validate new values
            if not isinstance(new_posicao,
                              int) or new_posicao < 1 or new_posicao > 1000:
                return {
                    "success": False,
                    "message": "Posição deve estar entre 1 e 1000"}
            ...
            # Update record
            c.execute("""UPDATE posicoes_participantes
                   SET posicao = %s, temporada = %s, data_atualizacao = %s
                   WHERE id = %s""", (new_posicao, new_temporada,
                                      datetime.now().isoformat(), registro_id))
            conn.commit()

            logger.info(
                f"✅ Resultado editado: id={registro_id}, posicao={new_posicao}, temporada={new_temporada}")
        ...
        return {
            "success": False,
            "message": f"Erro ao editar resultado: {
                str(e)}"}
...
def deletar_resultado_historico(registro_id: int) -> dict:
    ...
    try:
        with db_connect() as conn:
            ...
            usuario_id, temporada = existing['usuario_id'], existing['temporada']

            # Delete record
            c.execute(
                "DELETE FROM posicoes_participantes WHERE id = %s", (registro_id,))
            conn.commit()

            logger.info(
                f"✅ Resultado deletado: id={registro_id}, usuario_id={usuario_id}, temporada={temporada}")
        ...
        return {
            "success": False,
            "message": f"Erro ao deletar resultado: {
                str(e)}"}
...
def importar_resultados_em_lote(dados: list) -> dict:
    ...
    try:
        with db_connect() as conn:
            ...
            # Carrega chaves existentes em uma única leitura para evitar SELECT
            # por item
            c.execute("SELECT usuario_id, temporada FROM posicoes_participantes")
            existing_keys = {(int(r['usuario_id']), str(
                r['temporada'])) for r in c.fetchall()}
            ...
            for item in dados:
                ...
                if key in existing_keys:
                    ...
                    continue
                batch_values.append(
                    (item['usuario_id'], item['posicao'], item['temporada'], now_iso))
            ...
            logger.info(
                f"✅ Importação em lote: {imported} importados, {skipped} ignorados")
