...
def _on_prova_change():
    ...
    keys_to_clear = [
        "edit_nome_prova_val",
        "edit_data_prova_val",
        "edit_horario_prova_val",
        "edit_status_prova_val",
        "edit_tipo_prova_val",
        "edit_temporada_prova_val"]
...
def _render_aba_editar(df: pd.DataFrame):
    ...
    if df.empty:
        st.warning(
            "Não há provas para editar. Adicione uma prova na aba 'Adicionar Nova Prova'.")
        return
    ...
    try:
        horario_time = pd.to_datetime(
            str(horario_atual), format="%H:%M:%S").time()
    except Exception:
    ...
    tipo_index = tipo_options.index(
        tipo_atual) if tipo_atual in tipo_options else 0
    ...
    temporada_index = temporadas.index(
        temporada_atual) if temporada_atual in temporadas else 0
...
def _render_aba_adicionar():
    ...
    status_novo = st.selectbox(
        "Status", [
            "Ativa", "Inativa"], key="novo_status_prova")
    tipo_novo = st.selectbox(
        "Tipo", ["Normal", "Sprint"], key="novo_tipo_prova")
    ...
    circuito_sel_label = st.selectbox(
        "Circuito (base Ergast/Jolpica)",
        opcoes_circuito,
        index=0,
        key="novo_circuito_prova")
    circuito_sel_id = mapa_circuito.get(circuito_sel_label)

    # Temporada
    current_year = str(datetime.now().year)
    temporadas_add = get_season_options()
    temporada_index_add = get_default_season_index(
        temporadas_add, current_year=current_year)
    temporada_nova = st.selectbox(
        "Temporada",
        temporadas_add,
        index=temporada_index_add,
        key="nova_temporada_prova")
    ...
    if st.button("➕ Adicionar prova", key="btn_add_prova", type="primary"):
        if not nome_novo:
            ...
            with db_connect() as conn:
                ...
                if c.fetchone()['cnt'] > 0:
                    st.error(
                        f"❌ Já existe uma prova cadastrada com este nome e data para a temporada {temporada_nova}.")
                else:
                    # Inserir nova prova
                    if "temporada" in cols and "circuit_id" in cols:
                        c.execute(
...
def main():
    ...
    default_index = get_default_season_index(
        temporadas, current_year=current_year)
    temporada_sel = st.selectbox(
        "Temporada",
        temporadas,
        index=default_index,
        key="gestao_provas_temporada")

    if st.button(
        "🔄 Atualiza Base de Circuitos",
            key="btn_atualiza_base_circuitos"):
        ...
        st.success(
            f"✅ Base de circuitos atualizada: {
                stats.get(
                    'circuitos',
                    0)} circuitos de {
                stats.get(
    ...
    # Buscar provas filtradas por temporada usando helper compatível com
    # psycopg3
    df = get_provas_df(temporada_sel)
    df = _normalizar_df_provas(df)

    # Criar abas
    tab_editar, tab_adicionar = st.tabs(
        ["✏️ Editar Provas", "➕ Adicionar Nova Prova"])
