...
def _table_height(
    ...
    return _controller_table_height(
        total_rows,
...
def hall_da_fama():
    ...
    st.write(
        "📈 Histórico de classificações por temporada - Melhores posições em cada ano")
    ...
    with db_connect() as conn:
        ...
        for _, user in usuarios.iterrows():
            ...
            for season in seasons:
                ...
                if result and result.get('posicao') is not None:
                    ...
                    pts_val = result.get('pontos') if result.get(
                        'pontos') is not None else 0
        ...
        def score_user(row):
            # Filtra apenas valores que são números (posições), ignorando
            # 'Participante' e outros campos
            positions = []
        ...
        with col1:
            ...
            if 'perfil' in usuarios.columns:
                participantes_count = len(
                    usuarios[usuarios['perfil'].str.lower() != 'master'])
            else:
        ...
        with col3:
            # Maiores vencedores: top 3 participantes com mais temporadas
            # ganhas (1º lugar)
                ...
                season_stats.append(
                    {
                        ...
                        'Maior Pontuação': f"{
                            result['best_points']:.1f}" if result['best_points'] is not None else "-",
                        'Pontuação Média': f"{
                            result['avg_points']:.1f}" if result['avg_points'] is not None else "-"})
        ...
        if season_stats:
            ...
            st.dataframe(
                ...
                height=_table_height(
                    ...
                    max_height=520),
                column_config={
                    "Temporada": st.column_config.TextColumn(
                        "Temporada",
                        width="small"),
                    "Participantes": st.column_config.NumberColumn(
                        ...
                        format="%d",
                        width="small"),
                    "Maior Pontuação": st.column_config.TextColumn(
                        "Maior Pontuação",
                        width="small"),
                    "Pontuação Média": st.column_config.TextColumn(
                        "Pontuação Média",
                        width="small"),
                },
            )
        ...
        if rows:
            ...
            pivot = df_pos_counts.pivot_table(
                index='nome', columns='posicao', aggfunc=len, fill_value=0)
            # Sort columns by position
            pivot = pivot.reindex(sorted(pivot.columns), axis=1)
            pivot_config = {
                "_index": st.column_config.TextColumn(
                    "Participante", width="medium"), **{
                    col: st.column_config.NumberColumn(
                        f"{col}º", format="%d", width="small") for col in pivot.columns}, }
            st.dataframe(
                pivot,
                width="stretch",
            ...
            # Stacked bar chart: X=participante, Y=contagem de cada posição
            # (empilhadas)
            try:
...
def render_admin_panel(conn, seasons):
    ...
    if usuarios.empty:
        st.error(
            "❌ Não há usuários cadastrados no sistema. Cadastre usuários primeiro na seção 'Gestão de Usuários'.")
        return
    ...
    with tab1:
        ...
        if 'hall_fama_rows' not in st.session_state:
            st.session_state.hall_fama_rows = [
                {'user': None, 'position': None}] * 3

        st.write("**Participantes e Posições:**")
        st.write(
            "*Preencha quantas linhas forem necessárias. Novas linhas aparecerão automaticamente.*")
        ...
        for i in range(max_rows):
            ...
            if selected_user:
                entries.append(
                    {'user': selected_user, 'position': position, 'points': pontos})

                # Se a linha atual foi preenchida e é a última, adiciona mais 3
                # linhas
                if i == max_rows - \
                        1 and len(st.session_state.hall_fama_rows) < 50:
                    st.session_state.hall_fama_rows.extend(
                        [{'user': None, 'position': None}] * 3)
                    st.rerun()
        ...
        with col_btn1:
            if st.button(
                ...
                if not entries:
                    st.error(
                        "❌ Por favor, preencha pelo menos um participante e posição.")
                else:
                    errors = []
                    success_count = 0
                    user_id_by_name = {
                    ...
                    for entry in entries:
                        try:
                            ...
                            if not user_id:
                                errors.append(
                                    f"❌ **{user_name}**: usuário não encontrado")
                                continue
                    ...
                    if success_count > 0:
                        st.success(
                            f"✅ {success_count} resultado(s) adicionado(s) com sucesso!")
                        st.balloons()
                        st.cache_data.clear()
                        st.session_state.hall_fama_rows = [
                            {'user': None, 'position': None}] * 3
    ...
    with tab2:
        ...
        with col_filter1:
            filter_season = st.selectbox(
                ...
                sorted(
                    seasons,
                    reverse=True) if seasons else ["Todas"],
                key="filter_season")
        ...
        if records:
            ...
            for rec in records:
                record_id, name, position, points, season = rec['id'], rec[
                    'nome'], rec['posicao_final'], rec['pontos'], rec['temporada']
                with st.container():
                    ...
                    with col5:
                        if st.button(
                            ...
                            try:
                                c.execute(
                                    "DELETE FROM hall_da_fama WHERE id = %s", (record_id,))
                                conn.commit()
                                st.success(
                                    f"✅ Registro de **{name}** ({season}) deletado!")
                                st.cache_data.clear()
                                st.rerun()
                            except Exception as e:
...
def import_historical_data(conn):
    ...
    for idx, (temporada, usuario_id, posicao) in enumerate(historical_data):
        ...
        try:
            batch_values.append(
                (usuario_id, int(posicao), 0.0, str(temporada)))
            imported += 1
        except Exception as e:
            errors.append(
                f"usuario_id={usuario_id}, temporada={temporada}: {
                    str(e)}")
            skipped += 1
