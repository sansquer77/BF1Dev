...
def resultados_view():
    ...
    default_index = get_default_season_index(
        temporadas, current_year=current_year_str)
    ...
    if not resultado_atual.empty:
        try:
            posicoes_existentes = ast.literal_eval(
                resultado_atual.iloc[0]['posicoes']) or {}
        except Exception:
            posicoes_existentes = {}
        if 'abandono_pilotos' in resultados_df.columns:
            ...
            if raw_aband is None:
                raw_aband = ''
            abandonos_existentes = [p.strip() for p in str(
                raw_aband).split(',') if p and p.strip()]

    if st.session_state.get('resultados_prova_sel') != prova_id:
        st.session_state['resultados_prova_sel'] = prova_id
        for pos in range(1, 12):
            st.session_state[f"res_pos_{pos}"] = posicoes_existentes.get(
                pos, "")
    ...
    for pos in range(1, 6):
        with col1:
            ...
            opcoes = [
                ""] + [p for p in pilotos if p not in pilotos_usados or p == atual]
    ...
    if st.button("Salvar resultado"):
        ...
        # Validação opcional: alertar se algum abandonou e também está em top
        # 11
        conflitos = set(abandono_pilotos) & set(
            [posicoes.get(p) for p in range(1, 12) if posicoes.get(p)])
        if conflitos:
            st.warning(
                f"Pilotos em conflito (posicionados e como abandono): {
                    ', '.join(
                        sorted(conflitos))}")
        if erro:
            st.error(erro)
            ...
            with db_connect() as conn:
                ...
                abandono_str = ','.join(
                    abandono_pilotos) if abandono_pilotos else ''

                if 'temporada' in cols:
                    c.execute(
                        'SELECT 1 FROM resultados WHERE prova_id=%s AND (temporada=%s OR temporada IS NULL)',
                        (prova_id,
                    ...
                    c.execute(
                        'SELECT 1 FROM resultados WHERE prova_id=%s', (prova_id,))
                    existe = c.fetchone() is not None
                    if 'abandono_pilotos' in cols:
                        if existe:
                            c.execute(
