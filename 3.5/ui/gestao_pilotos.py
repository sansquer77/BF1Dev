...
def _normalizar_df_pilotos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Remove linhas com id não numérico (ex.: valor textual "id" vindo de
...
def _render_aba_editar(df: pd.DataFrame):
    ...
    if df.empty:
        st.warning(
            "Não há pilotos para editar. Adicione um piloto na aba 'Adicionar Novo Piloto'.")
        return
    ...
    numero_atual = int(piloto_row.get("numero", 0)) if piloto_row.get(
        "numero") is not None else 0
    ...
    status_index = status_options.index(
        status_atual) if status_atual in status_options else 0
        ...
        if st.button(
            ...
            with db_connect() as conn:
                ...
                if novo_status == "Ativo":
                    ...
                    if c.fetchone():
                        st.error(
                            "❌ Já existe outro piloto ativo com este nome.")
                        return
                else:
                    c.execute(
                    ...
                    if c.fetchone():
                        st.error(
                            "❌ Já existe outro piloto inativo com este nome, número e equipe.")
                        return
...
def _render_aba_adicionar():
    ...
    numero_novo = st.number_input(
        "Número do piloto",
        min_value=0,
        step=1,
        key="novo_numero_piloto")
    equipe_nova = st.text_input(
        "Equipe do novo piloto",
        key="nova_equipe_piloto")
    status_novo = st.selectbox(
        "Status", [
            "Ativo", "Inativo"], key="novo_status_piloto")
...
def main():
    ...
    tab_editar, tab_adicionar = st.tabs(
        ["✏️ Editar Pilotos", "➕ Adicionar Novo Piloto"])
