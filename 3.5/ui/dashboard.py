...
def main():
    ...
    with col1:
        ...
        try:
            ...
            default_index = years.index(
                int(api_current)) if api_current.isdigit() else 0
    ...
    with col2:
        ...
        st.markdown(
            ("<div style='display:inline-block;padding:6px 12px;border-radius:999px;"
             ...
             "color:#ffffff;font-weight:700;line-height:1.2;'>" f"{selected_season}" "</div>"),
                ...
                column_config={
                    **_cfg_position_points(),
                    "Driver": st.column_config.TextColumn(
                        "Piloto",
                        width="medium"),
                    "Constructor": st.column_config.TextColumn(
                        "Equipe",
                        width="medium"),
    ...
    try:
        ...
        if constructor_standings.empty:
            if selected_season < 1958:
                st.warning(
                    "⚠️ O Campeonato de Construtores foi criado apenas em 1958.")
            else:
                st.info("📅 Dados não disponíveis para esta temporada.")
        else:
            st.dataframe(
            ...
            if len(constructor_standings) > 0:
                ...
                st.success(
                    f"🏆 **Construtor Campeão {selected_season}**: {
                        constructor_champion['Constructor']} - {
                        constructor_champion['Points']} pontos")
    except Exception as e:
    ...
    try:
        ...
        if points_df.empty or len(points_df.columns) <= 2:
            ...
            # Limitar número de pilotos exibidos no gráfico para melhor
            # visualização
            ...
            if not chart_data.empty:
                ...
                if len(driver_cols) > 10:
                    st.info(
                        "📊 Exibindo top 10 pilotos. Use o filtro abaixo para personalizar.")
                    # Pegar top 10 por pontos finais
                    top_drivers = chart_data.iloc[-1].nlargest(
                        10).index.tolist()
    ...
    try:
        ...
        if delta_df.empty:
            ...
            st.dataframe(
                delta_df, width="stretch", hide_index=True, height=_table_height(
                    len(delta_df)), column_config={
                    "Driver": st.column_config.TextColumn(
                        "Piloto", width="medium"), "Qualifying": st.column_config.NumberColumn(
                        "Largada", format="%d", width="small"), "Race": st.column_config.NumberColumn(
                        "Chegada", format="%d", width="small"), "Delta": st.column_config.NumberColumn(
                            "Delta", format="%d", width="small"), }, )
            ...
            if len(delta_df) > 0:
                ...
                with col_gain:
                    st.success(
                        f"🚀 **Maior Subida**: {max_gain['Driver']} (+{max_gain['Delta']} posições)")
                with col_loss:
                    if max_loss['Delta'] < 0:
                        st.error(
                            f"🔻 **Maior Queda**: {max_loss['Driver']} ({max_loss['Delta']} posições)")
    except Exception as e:
    ...
    try:
        ...
        if fastest_laps.empty:
            if selected_season < 2004:
                st.warning(
                    "⚠️ Dados de volta mais rápida estão limitados para temporadas antigas.")
            else:
                st.info(
                    "📅 Nenhuma corrida realizada ainda ou dados não disponíveis.")
        else:
            st.dataframe(
                fastest_laps, width="stretch", hide_index=True, height=_table_height(
                    len(fastest_laps)), column_config={
                    "Driver": st.column_config.TextColumn(
                        "Piloto", width="medium"), "Fastest Lap": st.column_config.TextColumn(
                        "Volta", width="small"), }, )
            # Destacar volta mais rápida
            if len(fastest_laps) > 0:
                fastest = fastest_laps.iloc[0]
                st.success(
                    f"🏁 **Volta Mais Rápida**: {fastest['Driver']} - {fastest['Fastest Lap']}")
    except Exception as e:
    ...
    try:
        ...
        if pit_stops.empty:
            if selected_season < 2011:
                st.warning(
                    "⚠️ Dados de pit stops estão disponíveis apenas a partir da temporada 2011.")
            else:
                st.info(
                    "📅 Nenhuma corrida realizada ainda ou dados não disponíveis.")
        else:
