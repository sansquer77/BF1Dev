...
@pytest.fixture()
def prova_normal_df():
    return pd.DataFrame([{"id": 1, "nome": "GP Brasil", "data": "2025-11-02", "tipo": "Normal", "temporada": "2025"}])


@pytest.fixture()
def prova_sprint_df():
    return pd.DataFrame([{"id": 2, "nome": "GP Sprint", "data": "2025-11-01", "tipo": "Sprint", "temporada": "2025"}])
...
def resultado_normal_df():
    ...
    return pd.DataFrame([{"prova_id": 1, "posicoes": str(posicoes), "abandono_pilotos": ""}])
...
def resultado_sprint_df():
    posicoes = {1: "Verstappen", 2: "Hamilton", 3: "Leclerc", 4: "Sainz",
                5: "Norris", 6: "Russell", 7: "Piastri", 8: "Alonso",
                11: "Bottas"}
    return pd.DataFrame([{"prova_id": 2, "posicoes": str(posicoes), "abandono_pilotos": ""}])
...
class TestCalcularPontuacaoLote:
    ...
    def test_prova_sem_resultado_retorna_none(self, prova_normal_df):
        ...
        ap = _aposta_df(1, ["Hamilton", "Verstappen", "Leclerc"], [7, 5, 3], "Bottas")
        res_vazio = pd.DataFrame(columns=["prova_id", "posicoes", "abandono_pilotos"])
        resultado = calcular_pontuacao_lote(ap, res_vazio, prova_normal_df)
        assert resultado == [None]

    def test_pontuacao_basica_corrida_normal(self, prova_normal_df, resultado_normal_df):
        """Hamilton 1º (25pts) × 7=175 | Verstappen 2º (18pts) × 5=90 | Leclerc 3º (15pts) × 3=45 → 310."""
        from services.bets_scoring import calcular_pontuacao_lote

        ap = _aposta_df(1, ["Hamilton", "Verstappen", "Leclerc"], [7, 5, 3], "Stroll")
        resultado = calcular_pontuacao_lote(ap, resultado_normal_df, prova_normal_df)
        assert resultado == [310.0]

    def test_bonus_11o_colocado_aplicado(self, prova_normal_df, resultado_normal_df):
    ...
    def test_bonus_11o_nao_aplicado_quando_errado(self, prova_normal_df, resultado_normal_df):
        """Bônus NÃO adicionado quando piloto_11 não coincide com 11º real."""
        from services.bets_scoring import calcular_pontuacao_lote

        ap = _aposta_df(1, ["Hamilton", "Verstappen", "Leclerc"], [7, 5, 3], "Perez")
        resultado = calcular_pontuacao_lote(ap, resultado_normal_df, prova_normal_df)
        assert resultado == [310.0]

    def test_piloto_fora_top10_nao_pontua(self, prova_normal_df, resultado_normal_df):
        """Piloto fora do top-10 não contribui para a pontuação."""
        from services.bets_scoring import calcular_pontuacao_lote

        ap = _aposta_df(1, ["Hamilton", "Bottas", "Leclerc"], [7, 5, 3], "Stroll")
        resultado = calcular_pontuacao_lote(ap, resultado_normal_df, prova_normal_df)
    ...
    def test_penalidade_abandono_aplicada(self, prova_normal_df, monkeypatch):
        ...
        regras = {**REGRAS_PADRAO, "penalidade_abandono": True, "pontos_penalidade": 10}
        monkeypatch.setattr("services.bets_scoring.get_regras_aplicaveis", lambda *a, **k: regras)

        posicoes = {1: "Hamilton", 2: "Verstappen", 3: "Leclerc", 11: "Bottas"}
        res_df = pd.DataFrame([{"prova_id": 1, "posicoes": str(posicoes), "abandono_pilotos": "Verstappen"}])
        ap = _aposta_df(1, ["Hamilton", "Verstappen", "Leclerc"], [7, 5, 3], "Bottas")
    ...
    def test_dobrada_sprint_dobra_pontuacao(self, prova_sprint_df, resultado_sprint_df, monkeypatch):
        ...
        ap = _aposta_df(2, ["Verstappen", "Hamilton", "Leclerc"], [7, 5, 3], "Stroll")
        resultado = calcular_pontuacao_lote(ap, resultado_sprint_df, prova_sprint_df)
        # Verstappen 1º=8 × 7=56 | Hamilton 2º=7 × 5=35 | Leclerc 3º=6 × 3=18 → 109 × 2 = 218
        assert resultado == [218.0]

    def test_penalidade_aposta_automatica_segunda_geracao(self, prova_normal_df, resultado_normal_df):
        """Aposta automática de 2ª geração recebe redução de 20%."""
        from services.bets_scoring import calcular_pontuacao_lote

        ap = _aposta_df(1, ["Hamilton", "Verstappen", "Leclerc"], [7, 5, 3], "Stroll", automatica=2)
        resultado = calcular_pontuacao_lote(ap, resultado_normal_df, prova_normal_df)
        assert resultado == [248.0]  # 310 * 0.80

    def test_aposta_automatica_primeira_geracao_sem_penalidade(self, prova_normal_df, resultado_normal_df):
        """Aposta automática de 1ª geração NÃO sofre penalidade percentual."""
        from services.bets_scoring import calcular_pontuacao_lote

        ap = _aposta_df(1, ["Hamilton", "Verstappen", "Leclerc"], [7, 5, 3], "Stroll", automatica=1)
        resultado = calcular_pontuacao_lote(ap, resultado_normal_df, prova_normal_df)
        assert resultado == [310.0]

    def test_lote_multiplas_apostas(self, prova_normal_df, resultado_normal_df):
        """Lote com duas apostas retorna lista com dois elementos independentes."""
        from services.bets_scoring import calcular_pontuacao_lote

        ap1 = _aposta_df(1, ["Hamilton", "Verstappen", "Leclerc"], [7, 5, 3], "Stroll")
        ap2 = _aposta_df(1, ["Norris", "Russell", "Piastri"], [5, 5, 5], "Stroll")
        ap2["usuario_id"] = 2
...
class TestMapearPrimeiraPorTemporada:
    ...
    def test_retorna_prova_mais_antiga_por_temporada(self):
        ...
        provs = pd.DataFrame([
            {"id": 10, "nome": "GP A", "data": "2025-03-15", "tipo": "Normal", "temporada": "2025"},
            {"id": 11, "nome": "GP B", "data": "2025-04-20", "tipo": "Normal", "temporada": "2025"},
            {"id": 20, "nome": "GP C", "data": "2024-11-20", "tipo": "Normal", "temporada": "2024"},
        ])
    ...
    def test_sem_coluna_data_retorna_dict_vazio(self):
        ...
        provs = pd.DataFrame([{"id": 1, "nome": "GP X", "tipo": "Normal", "temporada": "2025"}])
        resultado = _mapear_primeira_prova_por_temporada(provs)
        assert resultado == {}
...
class TestOrdenarEPosicionar:
    ...
    def test_ordena_por_pontos_decrescente(self):
        ...
        tab = [
            {"usuario_id": 1, "pontos": 100.0, "acerto_11": 0, "data_envio": "2025-11-01 10:00:00"},
            {"usuario_id": 2, "pontos": 200.0, "acerto_11": 0, "data_envio": "2025-11-01 09:00:00"},
            {"usuario_id": 3, "pontos": 150.0, "acerto_11": 0, "data_envio": "2025-11-01 11:00:00"},
        ]
    ...
    def test_empate_desempatado_por_acerto_11(self):
        ...
        tab = [
            {"usuario_id": 1, "pontos": 100.0, "acerto_11": 0, "data_envio": "2025-11-01 10:00:00"},
            {"usuario_id": 2, "pontos": 100.0, "acerto_11": 1, "data_envio": "2025-11-01 09:00:00"},
        ]
        df = _ordenar_e_posicionar(tab)
    ...
    def test_empate_desempatado_por_data_envio_mais_antiga(self):
        ...
        tab = [
            {"usuario_id": 1, "pontos": 100.0, "acerto_11": 1, "data_envio": "2025-11-01 12:00:00"},
            {"usuario_id": 2, "pontos": 100.0, "acerto_11": 1, "data_envio": "2025-11-01 09:00:00"},
        ]
        df = _ordenar_e_posicionar(tab)
        assert df.iloc[0]["usuario_id"] == 2
