...
def salvar_aposta(
    ...
    horario_limite = datetime.strptime(
        f"{data_prova} {horario_prova}",
        '%Y-%m-%d %H:%M:%S')
    horario_limite = horario_limite.replace(
        tzinfo=ZoneInfo("America/Sao_Paulo"))

    # Horário atual normalizado para SP (ou horário forçado para aposta
    # automática)
    ...
    try:
        ...
        if tipo_aposta == 0:
            ...
            c.execute(
                '''INSERT INTO apostas (usuario_id, prova_id, data_envio, pilotos, fichas, piloto_11, nome_prova, automatica)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                (usuario_id,
                 prova_id,
                 data_envio,
    ...
    if total_fichas < n_pilotos:
        ...
        for i in range(n_pilotos):
            ...
            if i == n_pilotos - 1:
                ...
                # Máximo 10 fichas por piloto (1 já foi dada)
                max_para_este = min(9, total_fichas)
...
def gerar_aposta_automatica(
    ...
    if not prova_ant.empty:
        ...
        if not ap_ant.empty:
            ...
            st.success(
                f"Copiando aposta anterior da prova {
                    prova_ant['nome'].values[0]}")

    # Se não encontrou aposta anterior
    if pilotos_ant is None:
        pilotos_ant, fichas_ant, piloto_11_ant = gerar_aposta_aleatoria(
            pilotos_df)
    ...
    c = conn.cursor()
    c.execute(
        'SELECT id FROM apostas WHERE usuario_id=? AND prova_id=?',
        (usuario_id,
         prova_id))
    ...
    c = conn.cursor()
    c.execute(
        'SELECT MAX(automatica) FROM apostas WHERE usuario_id = ?', (usuario_id,))
...
def calcular_pontuacao_lote(apostas_df, resultados_df, provas_df):
    ...
    tipos_prova = dict(
        zip(
            provas_df['id'],
            provas_df['tipo'] if 'tipo' in provas_df.columns else ['Normal'] *
            len(provas_df)))
    pontos = []
...
def atualizar_classificacoes_todas_as_provas():
    ...
    usuarios_df = pd.read_sql(
        'SELECT * FROM usuarios WHERE status = "Ativo"', conn)
    ...
    for _, prova in provas_df.iterrows():
        ...
        for _, usuario in usuarios_df.iterrows():
            aposta = apostas_prova[apostas_prova['usuario_id']
                                   == usuario['id']]
            if aposta.empty:
                pontos = 0
                data_envio = None
                ...
                pontos = calcular_pontuacao_lote(
                    aposta, resultados_df, provas_df)
                ...
                if not aposta.empty and not resultados_df.empty:
                    ...
                    if not resultado.empty:
                        import ast
                        posicoes = ast.literal_eval(
                            resultado.iloc[0]['posicoes'])
        ...
        # Para garantir ordenação correta, converte data_envio para datetime
        # (None vai para o fim)
        df_classificacao['data_envio'] = pd.to_datetime(
            df_classificacao['data_envio'], errors='coerce')
    ...
    if not st.session_state['esqueceu_senha'] and not st.session_state['criar_usuario']:
        ...
        with col1:
            if st.button("Entrar"):
                ...
                if user:
                    ...
                    # user[0] deve ser o ID do usuário
        ...
        if st.button("Cadastrar usuário"):
            if cadastrar_usuario(
                    ...
                    status='Inativo'):
                st.success(
                    "Usuário criado com sucesso! Aguarde aprovação do administrador.")
                st.session_state['criar_usuario'] = False
            else:
...
if st.session_state['pagina'] == "Painel do Participante" and st.session_state['token']:
    ...
    if user[4] == "Ativo":
        if len(provas) > 0 and len(pilotos_df) > 2:
            prova_id = st.selectbox("Escolha a prova",
                                    provas['id'],
    ...
    if not posicoes_part.empty:
        # Certifique-se de que provas_df está definido e atualizado antes deste
        # bloco
...
if st.session_state['pagina'] == "Gestão de Usuários" and st.session_state['token']:
    ...
    if payload['perfil'] == 'master':
        ...
        if len(usuarios) == 0:
            ...
            st.write(
                "Selecione um usuário para editar, excluir ou alterar status/perfil:")
            usuario_id = st.selectbox("Usuário", usuarios['id'])
            usuario = usuarios[usuarios['id'] == usuario_id].iloc[0]
            novo_nome = st.text_input(
                "Nome", value=usuario['nome'], key="edit_nome")
            novo_email = st.text_input(
                "Email", value=usuario['email'], key="edit_email")
            novo_status = st.selectbox(
                "Status", [
                    "Ativo", "Inativo"], index=0 if usuario['status'] == "Ativo" else 1, key="edit_status")
            novo_perfil = st.selectbox("Perfil",
                                       ["participante",
                                        "admin"],
                                       index=0 if usuario['perfil'] == "participante" else 1,
                                       key="edit_perfil")
            ...
            with col1:
                if st.button("Atualizar usuário"):
                    if usuario['nome'] == "Password":
                        st.warning(
                            "Não é permitido alterar o status ou perfil do usuário master.")
                    else:
                        conn = db_connect()
                        c = conn.cursor()
...
if st.session_state['pagina'] == "Gestão do campeonato" and st.session_state['token']:
    ...
    if payload['perfil'] == 'master':
        ...
        with tab1:
            ...
            nome_piloto = st.text_input(
                "Nome do novo piloto", key="nome_novo_piloto")
            equipe_piloto = st.text_input(
                "Nome da equipe do piloto",
                key="equipe_novo_piloto")
            status_piloto = st.selectbox(
                "Status do piloto", [
                    "Ativo", "Inativo"], key="status_novo_piloto")
            if st.button("Adicionar piloto", key="btn_add_piloto_form"):
                if not nome_piloto.strip():
                    ...
                    c = conn.cursor()
                    c.execute(
                        'INSERT INTO pilotos (nome, equipe, status) VALUES (?, ?, ?)',
                        (nome_piloto.strip(),
                         equipe_piloto.strip(),
                         status_piloto))
                    conn.commit()
            ...
            if len(pilotos) == 0:
                ...
                for idx, row in pilotos.iterrows():
                    ...
                    with col1:
                        novo_nome = st.text_input(
                            f"Nome piloto {
                                row['id']}", value=row['nome'], key=f"pl_nome_{
                                row['id']}")
                    with col2:
                        nova_equipe = st.text_input(
                            f"Equipe piloto {
                                row['id']}", value=row['equipe'], key=f"pl_eq_{
                                row['id']}")
                    with col3:
                        novo_status = st.selectbox(
                            f"Status piloto {
                                row['id']}", [
                                "Ativo", "Inativo"], index=0 if row.get(
                                'status', 'Ativo') == "Ativo" else 1, key=f"pl_status_{
                                row['id']}")
                    with col4:
                        if st.button(
                            "Editar piloto", key=f"pl_edit_{
                                row['id']}"):
                            conn = db_connect()
                            c = conn.cursor()
                            c.execute(
                    ...
                    with col5:
                        if st.button(
                            "Excluir piloto", key=f"pl_del_{
                                row['id']}"):
                            conn = db_connect()
                            c = conn.cursor()
                            c.execute(
                                'DELETE FROM pilotos WHERE id=?', (row['id'],))
                            conn.commit()
                            conn.close()
        ...
        with tab2:
            ...
            nome_prova = st.text_input(
                "Nome da nova prova", key="nome_nova_prova")
            data_prova = st.date_input(
                "Data da nova prova", key="data_nova_prova")
            horario_prova = st.time_input(
                "Horário da nova prova", value=time(
                    15, 0), key="hora_nova_prova")  # valor padrão: 15:00
            status_prova = st.selectbox(
                "Status da prova", [
                    "Ativo", "Inativo"], key="status_nova_prova")
            tipo_prova = st.selectbox(
                "Tipo da prova", [
                    "Normal", "Sprint"], key="tipo_nova_prova")
            ...
            if st.button("Adicionar prova", key="btn_add_prova_form"):
                if not nome_prova.strip():
                    ...
                    try:
                        ...
                        c = conn.cursor()
                        c.execute(
                            'INSERT INTO provas (nome, data, horario_prova, status, tipo) VALUES (?, ?, ?, ?, ?)',
                            (nome_prova.strip(),
                             data_str,
                             hora_str,
                             status_prova,
            ...
            if len(provas) == 0:
                ...
                for idx, row in provas.iterrows():
                    ...
                    with col1:
                        novo_nome = st.text_input(
                            f"Nome prova {
                                row['id']}", value=row['nome'], key=f"pr_nome_{
                                row['id']}")
                    with col2:
                        # Data
                        try:
                            data_val = pd.to_datetime(
                                row['data']).date() if pd.notnull(
                                row['data']) else datetime.today().date()
                        except Exception:
                            data_val = datetime.today().date()
                        nova_data = st.date_input(
                            f"Data prova {
                                row['id']}", value=data_val, key=f"pr_data_{
                                row['id']}")
                    with col3:
                        # Horário
                        try:
                            hora_val = pd.to_datetime(
                                row['horario_prova']).time() if pd.notnull(
                                row['horario_prova']) else time(
                                15,
                                0)
                        except Exception:
                            hora_val = time(15, 0)
                        novo_horario = st.time_input(
                            f"Horário prova {
                                row['id']}", value=hora_val, key=f"pr_hora_{
                                row['id']}")
                    with col4:
                        novo_status = st.selectbox(
                            f"Status prova {
                                row['id']}", [
                                "Ativo", "Inativo"], index=0 if row.get(
                                'status', 'Ativo') == "Ativo" else 1, key=f"pr_status_{
                                row['id']}")
                    with col5:
                        novo_tipo = st.selectbox(
                            f"Tipo prova {
                                row['id']}", [
                                "Normal", "Sprint"], index=0 if row.get(
                                'tipo', 'Normal') == "Normal" else 1, key=f"pr_tipo_{
                                row['id']}")
                    with col6:
                        if st.button(
                            "Editar prova", key=f"pr_edit_{
                                row['id']}"):
                            try:
                    ...
                    with col6:
                        if st.button(
                            "Excluir prova", key=f"pr_del_{
                                row['id']}"):
                            conn = db_connect()
                            c = conn.cursor()
                            c.execute(
                                'DELETE FROM provas WHERE id=?', (row['id'],))
                            conn.commit()
                            conn.close()
    ...
    if payload['perfil'] in ['master', 'admin']:
        ...
        for idx, part in enumerate(participantes.itertuples()):
            with tabs[idx]:
                ...
                apostas_dict = dict(
                    zip(apostas_part['prova_id'], apostas_part.itertuples()))
                for _, prova in provas_df.iterrows():
                    ...
                    if prova['id'] in apostas_dict:
                        ...
                        if st.button(
                            f"Gerar aposta automática para {
                                prova['nome']}", key=f"auto_{
                                part.id}_{
                                prova['id']}"):
                            ...
                            c = conn.cursor()
                            c.execute(
                                'SELECT faltas FROM usuarios WHERE id=?', (part.id,))
                            faltas_atual = c.fetchone()
                            faltas_novo = (
                                faltas_atual[0] if faltas_atual and faltas_atual[0] else 0) + 1
                            c.execute(
                                'UPDATE usuarios SET faltas=? WHERE id=?', (faltas_novo, part.id))
                            conn.commit()
                            conn.close()

                            # Tenta gerar aposta automática
                            ok, msg = gerar_aposta_automatica(
                                part.id, prova['id'], prova['nome'], apostas_df, provas_df)

                            if not ok:
                                # Fallback para aposta zerada
                                resultado_row = resultados_df[resultados_df['prova_id']
                                                              == prova['id']]
                                if not resultado_row.empty:
                                    ...
                                    pilotos_nao_pontuaram = list(
                                        todos_pilotos - pontuaram)
                                    piloto_11 = resultado.get("11", None)
                                    pilotos_11_opcoes = [
                                        p for p in todos_pilotos if p != piloto_11]
                                    piloto_11_nao = pilotos_11_opcoes[0] if pilotos_11_opcoes else list(todos_pilotos)[
                                        0]
                                else:
                                    pilotos_nao_pontuaram = [
                                        pilotos_df['nome'].iloc[0]]
                                    piloto_11_nao = pilotos_df['nome'].iloc[0]

                                piloto_aposta = pilotos_nao_pontuaram[
                                    0] if pilotos_nao_pontuaram else pilotos_df['nome'].iloc[0]
...
if st.session_state['pagina'] == "Classificação" and st.session_state['token']:
    ...
    # --------- NOVO: Coluna de movimentação baseada na classificação acumulad
    ...
    df_class["Diferença"] = ["-" if i ==
                             0 else f"{d:.2f}" for i, d in enumerate(diferencas)]
    ...
    if payload['perfil'] in ['master', 'admin']:
        ...
        if len(provas) > 0 and len(pilotos_ativos_df) > 0:
            ...
            for pos in range(1, 6):
                with col1:
                    opcoes = [""] + \
                        [p for p in pilotos if p not in pilotos_usados]
                    piloto_sel = st.selectbox(
                        f"{pos}º colocado",
                        opcoes,
                        index=0,
            ...
            st.warning(
                "Cadastre provas e pilotos ativos antes de lançar resultados.")
    else:
        st.warning("Acesso restrito ao administrador/master.")
...
if st.session_state['pagina'] == "Log de Apostas" and st.session_state['token']:
    ...
    if payload['perfil'] == 'master':
        ...
        df = pd.read_sql(
            'SELECT * FROM log_apostas WHERE apostador=?',
            conn,
            params=(
                nome,
            ))
...
def importar_apostas_campeonato_excel(arquivo_excel_bytes):
    ...
    for _, row in df.iterrows():
        try:
            ...
            vice = row['Vice']
            equipe = row['Equipe']
            data_aposta = row.get('Data Aposta',
...
def modulo_exportar_importar_excel():
    ...
    st.header(
        "💾 Backup das Bases (Apostas / Pilotos / Provas / Resultados / Log de Apostas")
    ...
    if not arquivos_excel:
        ...
        for tabela, conteudo in arquivos_excel.items():
            st.download_button(
                ...
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    st.header(
        "⬆️ Importação das Bases (Apostas / Pilotos / Provas / Resultados / Log de Apostas")
    ...
    if tabelas:
        ...
        if arquivo and tabela_escolhida:
            if st.button("Importar dados"):
                try:
                    msg = importar_excel_para_tabela(
                        DB_PATH, tabela_escolhida, arquivo.read())
                    st.success(msg)
                except Exception as e:
