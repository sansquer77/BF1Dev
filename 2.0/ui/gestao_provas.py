...
def main():
    ...
    if df.empty:
        ...
        show_df = df[["id", "nome", "data",
                      "horario_prova", "tipo", "status"]].copy()
    ...
    if not df.empty:
        ...
        nova_data = st.date_input(
            ...
            datetime.strptime(
                prova_row["data"],
                "%Y-%m-%d") if prova_row["data"] else datetime.now(),
            key="edit_data")
        ...
        novo_tipo = st.selectbox("Tipo",
                                 ["Normal",
                                  "Sprint"],
                                 index=0 if prova_row["tipo"] == "Normal" else 1,
                                 key="edit_tipo")
        novo_status = st.selectbox("Status",
                                   ["Ativo",
                                    "Inativo"],
                                   index=0 if prova_row["status"] == "Ativo" else 1,
                                   key="edit_status")
        ...
        with col2:
            if st.button("Excluir prova", key="btn_delete_prova"):
                conn = db_connect()
                c = conn.cursor()
                c.execute("DELETE FROM provas WHERE id=?",
                          (int(prova_row["id"]),))
                conn.commit()
                conn.close()
    ...
    status_novo = st.selectbox(
        "Status", [
            "Ativo", "Inativo"], key="novo_status_prova")
