    ...
    if user[4] == "Ativo":
        if len(provas) > 0 and len(pilotos_df) > 2:
            ...
            piloto_11 = st.selectbox(
                ...
                index=pilotos_df['nome'].tolist().index(piloto_11_ant) if piloto_11_ant in pilotos_df['nome'].tolist() else 0
            )
            # Validação
            ...
            if st.button("Efetivar Aposta"):
                if len(equipes_apostadas) < 3:
                    ...
                    salvar_aposta(user[0], prova_id, pilotos_selecionados, [fichas[e] for e in equipes_apostadas], piloto_11, nome_prova)
